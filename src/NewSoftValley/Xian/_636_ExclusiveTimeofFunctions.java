...
public class _636_ExclusiveTimeofFunctions {
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        s.push(new function(-1,23,0));
        for(String log : logs){
            String [] content= log.split(":");
            int id = Integer.valueOf(content[0]);
            int time = Integer.valueOf(content[2]);
            if(content[1].equals("start")){
                s.push(new function(id,time,0));
            }
            else {
                int consume = time - s.peek()._start + 1;
                result[id] += time - s.peek()._start + 1 - s.peek()._consumeByChild;
                s.pop();
                s.peek()._consumeByChild += consume;
            }
        }
        return result;
    }
    class function{
        ...
        public function(int id , int start,int consumeByChild){
            this._id = id ;
            this._start = start ;
            this._consumeByChild = consumeByChild;
