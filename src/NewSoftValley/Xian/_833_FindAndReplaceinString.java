...
public class _833_FindAndReplaceinString {
    public String findReplaceString(String S, int[] indexes, String[] sources, String[] targets) {
        ...
        for(int i = 0 ; i < indexes.length ; i++){
            hm.put(indexes[i],new String[]{sources[i],targets[i]});
        }
        StringBuilder sb = new StringBuilder();
        int start = 0 ;
        for(Map.Entry<Integer,String []> entry : hm.entrySet()){
            if(S.substring(entry.getKey(),entry.getKey() + entry.getValue()[0].length()).equals(entry.getValue()[0])){
                sb.append(S.substring(start,entry.getKey())).append(entry.getValue()[1]);
                start = entry.getKey() + entry.getValue()[0].length();
            }
        }
        if(start != S.length())
